...
拥有 `10` 年软件开发经验，其中 `8` 年专注于前端技术。精通 `React`、`Vue`、`TypeScript` 等主流前端技术，对 `DDD` 和 `OOP` 编程有一定的经验，擅长开发高性能、可扩展的大型复杂 `Web` 应用。具备丰富的项目经验，主导并参与过多个商业项目及创业项目，涵盖 _实时监控系统_、_图形编辑器_、`CMS`、`CRM`、`ERP`、电商系统等，对 `WMS`、`WES`、`WCS` 有深入了解。
