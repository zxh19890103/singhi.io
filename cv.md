...
拥有 `10` 年软件开发经验，其中 `8` 年专注于前端技术。精通 `React`、`Vue`、`TypeScript` 等主流前端技术，擅长开发高性能、可扩展的 `Web` 应用。具备丰富的项目经验，主导并参与过多个商业项目及创业项目，涵盖 _实时监控系统_、_图形编辑器_、`CMS`、`CRM`、`ERP`、电商系统等，对 `WMS`、`WES`、`WCS` 有深入了解。

在过去 `3` 年中，作为高级前端工程师在海柔创新科技专注于*仓库布局系统（编辑器）*、*仓库图形库*与*实时监控系统* 的研发，成果丰富，积累了深厚的图形领域知识，致力于成为图形化方向的前端技术专家。

具备良好的团队合作与管理经验，能快速适应并解决技术挑战，注重*代码规范*与*高效开发*，关注项目的长期*可维护性*与*可扩展性*。
...
#### `2023/09 - ` _|_ 个人项目及自主创业

- **3D 仓库监控系统** 开发了基于 `THREE.js` 的仓库监控系统，能够展示实时监控数据并提供演示。
- **照片编辑软件** 设计并开发了一款可以实时在线编辑图片的应用，支持多种图像处理功能。
- **地图相册应用** 基于 `leaflet` 和 `react` 以及 `Electron` 框架开发了一款结合地图和照片管理功能的应用，提供位置相关的照片浏览体验，同时支持桌面版和 web 版。
- **THREE.js 学习计划** 一个帮助初学者入门 `THREE.js` 的网站，提供丰富的交互，让用户从感官上理解 `THREE.js` 的各项配置。
- **工作室创办尝试** 尝试与输送线厂商合作，推出 3d 输送线监控软件。

#### `2021/05 - 2023/09` _|_ 深圳市海柔创新科技有限公司 _|_ 高级前端工程师

- 负责前端工程建设，设计与开发高性能 _仓库实时监控系统_ 及 _仓库图形编辑系统(WLS)_。
