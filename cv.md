...
拥有 `10` 年软件开发经验，`8` 年专注前端技术，擅长*面向对象编程*（`OOP`）和*架构设计*。精通 `React、Vue、TypeScript` 等主流前端技术，具备深入的领域驱动设计（`DDD`）经验。致力于构建高性能、可扩展的企业级 Web 应用，具有丰富的项目经验，涵盖实时监控系统、图形编辑器、`CMS`、`CRM`、`ERP`、电商系统等，深入理解 `WMS`、`WES` 和 `WCS` 等仓储管理系统。
