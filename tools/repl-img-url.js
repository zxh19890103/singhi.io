...
const uploadToOSS = async (incomingMsg, resName) => {
  ...
  return ossClient.putStream(filename, incomingMsg).then((res) => {
...
const circle = async () => {
  ...
  try {
    ...
    lines[lineno] = lines[lineno].replace(
      url,
      `//zxh1989.oss-cn-qingdao.aliyuncs.com/${ossUrl}`,
    );
  } catch (ex) {
    ...
    if (URLs.length === 0) {
      ...
      if (entries.length === 0) {
        ...
        fs.writeFileSync(posts_dir + "/" + nameOfPost, lines.join("\n"));
